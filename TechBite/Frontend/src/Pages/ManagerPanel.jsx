...
function ManagerPanel() {
  ...
  const addItem = async () => {
    try {
      await axios.post("http://localhost:3007/add-item", {
        tableName: category,
        itemName,
        price,
      });
  ...
  const updateItemPrice = async (tableName, itemName, newPrice) => {
    try {
      await axios.post("http://localhost:3007/update-item", {
        tableName,
        itemName,
        newPrice,
      });
              ...
              items
                ...
                .map((item, index) => (
                  <tr key={index}>
                    ...
                    <td
                      className={
                        editableItem === `${tableName}_${item.item_name}`
                          ? "editable"
                          : ""
                      }
                      contentEditable={
                      ...
                      onDoubleClick={() =>
                        setEditableItem(`${tableName}_${item.item_name}`)
                      }
                      onBlur={(e) => {
                    ...
                    <td>
                      <button
                        ...
                        onClick={() => removeItem(tableName, item.item_name)}
                      >
                        Remove
                      </button>
                    </td>
